...
import { addCheckoutDraftItem, readCheckoutDraft } from '../features/orders/draft';
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    let current = true;
    setProduct(undefined); setError(false); setLikeError(undefined); setQuantity(1); setDraftStatus(undefined); setDraftError(undefined); setDraftQuantity(readCheckoutDraft().find((item) => item.productId === productId)?.quantity ?? 0);
  ...
  async function toggleLike() {
    ...
    } finally { if (mounted.current) setLoadingLike(false); }
  }

  function addToCheckoutDraft(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!product || product.stock < 1) return;
    const existingQuantity = readCheckoutDraft().find((item) => item.productId === product.id)?.quantity ?? 0;
    const remainingStock = product.stock - existingQuantity;
    if (remainingStock < 1) {
      setDraftStatus(undefined);
      setDraftError(`이미 임시 주문서에 재고 ${product.stock.toLocaleString('ko-KR')}개가 담겨 있어 더 담을 수 없습니다.`);
      return;
    }
    if (!Number.isInteger(quantity) || quantity < 1 || quantity > remainingStock) {
      setDraftStatus(undefined);
      setDraftError(`현재 추가 수량은 1개부터 ${remainingStock.toLocaleString('ko-KR')}개까지 선택할 수 있습니다.`);
      return;
    }
    try {
      const nextItems = addCheckoutDraftItem({ productId: product.id, name: product.name, price: product.price, quantity });
      setDraftQuantity(nextItems.find((item) => item.productId === product.id)?.quantity ?? existingQuantity);
  ...
  const remainingStock = product ? Math.max(product.stock - draftQuantity, 0) : 0;
    ...
    {product && <article className="product-detail">
      ...
      <Card className="product-detail__content"><p className="page-kicker">{product.brand?.name ?? '브랜드 정보 없음'}</p><h1 id="product-detail-title">{product.name}</h1><p className="product-detail__price">{priceText(product.price)}</p><p className="product-card__meta">{product.category?.name ?? '카테고리 정보 없음'} · 재고 {product.stock.toLocaleString('ko-KR')}개 · 좋아요 {product.likeCount.toLocaleString('ko-KR')}개</p><p className="product-detail__description">{product.description || '상품 상세 설명을 준비하고 있습니다.'}</p>
        <form className="product-detail__draft" onSubmit={addToCheckoutDraft}>
          <label htmlFor="order-quantity">주문 수량<input className="ds-input" disabled={remainingStock < 1} id="order-quantity" max={product.stock} min="1" onChange={(event) => setQuantity(Number(event.target.value))} type="number" value={quantity} /></label>
          {product.stock < 1 ? <p className="product-card__meta">현재 재고가 없어 주문서에 담을 수 없습니다.</p> : remainingStock < 1 ? <p className="product-card__meta">현재 재고만큼 이미 임시 주문서에 담겼습니다.</p> : <p className="product-card__meta">이 브라우저 탭의 임시 주문서에만 담기며, 서버 장바구니가 아닙니다. 현재 {remainingStock.toLocaleString('ko-KR')}개를 더 담을 수 있습니다.</p>}
          <Button disabled={remainingStock < 1} type="submit">주문서에 담기</Button>
