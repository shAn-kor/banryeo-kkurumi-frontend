...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    let current = true;
    setProduct(undefined); setError(false); setLikeError(undefined);
  ...
  return <Page><Container><Section className="storefront-page" labelledBy="product-detail-title">
    ...
    {product && <article className="product-detail">
      ...
      <Card className="product-detail__content"><p className="page-kicker">{product.brand?.name ?? '브랜드 정보 없음'}</p><h1 id="product-detail-title">{product.name}</h1><p className="product-detail__price">{priceText(product.price)}</p><p className="product-card__meta">{product.category?.name ?? '카테고리 정보 없음'} · 재고 {product.stock.toLocaleString('ko-KR')}개 · 좋아요 {product.likeCount.toLocaleString('ko-KR')}개</p><p className="product-detail__description">{product.description || '상품 상세 설명을 준비하고 있습니다.'}</p>{likeError && <ErrorState>{likeError}</ErrorState>}<Button aria-pressed={liked} disabled={loadingLike || session.status === 'checking'} onClick={() => { void toggleLike(); }}>{loadingLike ? '좋아요 처리 중' : liked ? '좋아요 취소' : '좋아요'}</Button><Link className="inline-link" to="/products">상품 목록으로 돌아가기</Link></Card>
    </article>}
